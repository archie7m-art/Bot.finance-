...
export default async function handler(req,res){
 ...
 try{const redis=Redis.fromEnv(),u=await universe(redis);if(!u.length)return res.status(503).json({error:'No universe configured'});
  ...
  await redis.set('mrl:collector:state',JSON.stringify({date,offset:offset+symbols.length,total:u.length,success:out.length,errors:errors.length,lastRun:new Date().toISOString()}),{ex:60*60*24*30});
  return res.json({ok:true,date,processed:symbols.length,success:out.length,errors:errors.length,nextOffset:offset+symbols.length,total:u.length});
