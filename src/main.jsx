...
import './styles.css';
const demo=[
{symbol:'RELIANCE.BO',company:'Reliance Industries',close:1420,return1d:1.2,return5d:4.2,return20d:8.4,rvolZ:2.1,buyPressure:2.4,sellPressure:.3,netPressure:2.1,momentumZ:1.8,drawdown:-3.2,volatility:19.4,clv:.74,participation:1.8,pressureTrend:.9,state:'BUILD-UP',confidence:82,quality:.96,marketCap:1970806.49,rank:1,capTier:'Large'},
{symbol:'TCS.BO',company:'Tata Consultancy Services',close:3920,return1d:.2,return5d:1.1,return20d:2.3,rvolZ:.4,buyPressure:.5,sellPressure:.3,netPressure:.2,momentumZ:.6,drawdown:-4.8,volatility:18.2,clv:.15,participation:.3,pressureTrend:.1,state:'BALANCED',confidence:61,quality:.96,marketCap:1129582,rank:4,capTier:'Large'},
{symbol:'HDFCBANK.BO',company:'HDFC Bank',close:1680,return1d:-1.1,return5d:-2.2,return20d:-5.8,rvolZ:1.7,buyPressure:.2,sellPressure:1.9,netPressure:-1.7,momentumZ:-1.2,drawdown:-11.1,volatility:28.1,clv:-.71,participation:1.6,pressureTrend:-.8,state:'DISTRIBUTION',confidence:85,quality:.96,marketCap:1516760.98,rank:2,capTier:'Large'},
{symbol:'SBIN.BO',company:'State Bank of India',close:820,return1d:1.4,return5d:3.4,return20d:7.1,rvolZ:2.8,buyPressure:2.8,sellPressure:.4,netPressure:2.4,momentumZ:2.2,drawdown:-5.7,volatility:24.5,clv:.82,participation:2.4,pressureTrend:1.1,state:'BUILD-UP',confidence:89,quality:.96,marketCap:809938.33,rank:6,capTier:'Large'}];
...
function openFilePicker(){fileInputRef.current?.click()}
 ...
 return <div className="app"><header><div className="brand"><div className="mark">MR</div><div><h1>Market Research Lab <em>FINAL</em></h1><span>Multi-bot orchestrated research workstation</span></div></div><div className="status"><i/> {configured?'LIVE MATRIX':'RESEARCH MODE'} <span>•</span> DAILY</div></header>
 ...
 <main><section className="hero"><div><p className="eyebrow">OBSERVE → NORMALIZE → MODEL → VALIDATE</p><h2>Market behavior,<br/><b>reconstructed from matrices.</b></h2><p className="sub">An evidence-first quantitative engine that converts observable market data into pressure, participation, momentum, risk and latent-state matrices—then tests recurring configurations.</p></div><div className="heroTools">
  ...
  />
  <button className="refresh" onClick={()=>{load();loadValidation();loadOrchestrator()}}>
 ...
 <Orchestrator job={orchestrator?.job} registry={orchestrator?.registry||DEFAULT_BOTS} onOpen={setOpenBot}/>
 ...
 {selectedRow&&<Drawer row={selectedRow} onClose={()=>setSelected(null)} date={orchestrator?.date} registry={orchestrator?.registry||DEFAULT_BOTS}/>}
 {openBot&&<BotDetailDrawer botId={openBot} onClose={()=>setOpenBot(null)} date={orchestrator?.date} registry={orchestrator?.registry||DEFAULT_BOTS} job={orchestrator?.job}/>}</main></div>}
function Orchestrator({job,registry,onOpen}){return <section className="outputCard orchestrator"><div className="sectionTitle"><div><p className="eyebrow"><span className="liveDot"/>ORCHESTRATOR</p><h3>Bot task assignment & status</h3></div><Activity size={19}/></div><p className="description">Every security is handed through this fixed sequence of bots. Each bot logs its own verdict independently — nothing is merged silently, and the Risk Bot can cap confidence without rewriting anyone else's decision. Click a bot to open its live feed.</p><div className="patternGrid">{registry.map(b=>{const c=job?.botCounts?.[b.id];const live=(c?.error>0)||(c?.flagged>0);return <article className={'patternCard botCard'+(live?' isLive':'')} key={b.id} onClick={()=>onOpen(b.id)}><b>{b.name}</b><strong>{c?c.done.toLocaleString():'—'}</strong><span>{b.task}</span>{c?.error>0&&<span className="botErr">{c.error} error{c.error===1?'':'s'}</span>}{c?.flagged>0&&<span className="botFlag">{c.flagged} flagged</span>}</article>})}</div>{!job&&<div className="notice"><Info size={14}/>No orchestrator run recorded yet — seed a universe and run the collector (or connect Redis) to see live task counts.</div>}{job?.shards?.length>0&&<div className="shardLog"><b>Recent shard runs</b>{job.shards.slice(-6).reverse().map((sh,i)=><span key={i}>offset {sh.offset}: {sh.success}/{sh.processed} ok{sh.errors?`, ${sh.errors} failed`:''}</span>)}</div>}</section>}
...
function BotDetailDrawer({botId,onClose,date,registry,job}){
 const [feed,setFeed]=useState([]),[loading,setLoading]=useState(false);
 const meta=registry.find(b=>b.id===botId)||{name:botId,task:''};
 const counts=job?.botCounts?.[botId];
 useEffect(()=>{
  let cancelled=false;
  if(!date){setFeed([]);return}
  setLoading(true);
  fetch(`/api/bots?date=${encodeURIComponent(date)}&bot=${encodeURIComponent(botId)}`)
   .then(r=>r.ok?r.json():null)
   .then(j=>{if(!cancelled)setFeed(j?.feed||[])})
   .catch(()=>{if(!cancelled)setFeed([])})
   .finally(()=>{if(!cancelled)setLoading(false)});
  return ()=>{cancelled=true};
 },[botId,date]);
 return <div className="drawerBack" onClick={onClose}><aside className="drawer" onClick={e=>e.stopPropagation()}><button className="close" onClick={onClose}>×</button><p className="eyebrow"><span className="liveDot"/>BOT DETAIL</p><h3>{meta.name}</h3><span className="company">{meta.task}</span>
  <div className="stateHero"><span>THIS RUN</span><strong>{counts?`${counts.done} done`:'No run yet'}</strong><small>{counts?.error>0?`${counts.error} error${counts.error===1?'':'s'}`:''}{counts?.flagged>0?` · ${counts.flagged} flagged`:''}</small></div>
  <div className="botDetail"><b>Live feed — what this bot decided, per security</b>
   {!date&&<small>No live orchestrator run yet.</small>}
   {date&&loading&&<small>Loading…</small>}
   {date&&!loading&&feed.length===0&&<small>No entries logged for this bot on {date} yet.</small>}
   {feed.map((f,i)=><div className={'botFeedRow status-'+f.status} key={i}><b>{f.symbol}</b><span>{f.verdict}</span></div>)}
  </div>
 </aside></div>}
function Drawer({row,onClose,date,registry}){
 const [audit,setAudit]=useState([]),[auditLoading,setAuditLoading]=useState(false);
 useEffect(()=>{
  let cancelled=false;
  if(!date){setAudit([]);return}
  setAuditLoading(true);
  fetch(`/api/bots?date=${encodeURIComponent(date)}&symbol=${encodeURIComponent(row.symbol)}`)
   .then(r=>r.ok?r.json():null)
   .then(j=>{if(!cancelled)setAudit(j?.audit||[])})
   .catch(()=>{if(!cancelled)setAudit([])})
   .finally(()=>{if(!cancelled)setAuditLoading(false)});
  return ()=>{cancelled=true};
 },[row.symbol,date]);
 const evidence=[['Cap tier',row.capTier||'—'],['Market cap',row.marketCap?`₹${n(row.marketCap).toLocaleString(undefined,{maximumFractionDigits:0})} Cr`:'—'],['1D return',pct(row.return1d)],['5D return',pct(row.return5d)],['Volume anomaly',sig(row.rvolZ)],['Buy pressure',n(row.buyPressure).toFixed(2)],['Sell pressure',n(row.sellPressure).toFixed(2)],['Net pressure',n(row.netPressure).toFixed(2)],['Momentum',sig(row.momentumZ)],['Relative trend',sig(row.trendAlignment)],['Drawdown',pct(row.drawdown)],['Model confidence',`${n(row.confidence)}%`],['Data quality',`${Math.round(n(row.quality)*100)}%`]];
 return <div className="drawerBack" onClick={onClose}><aside className="drawer" onClick={e=>e.stopPropagation()}><button className="close" onClick={onClose}>×</button><p className="eyebrow">SECURITY EVIDENCE</p><h3>{row.symbol}</h3><span className="company">{row.company}</span><div className="stateHero"><span>MARKET STATE</span><strong>{row.state}</strong><small>Model confidence {n(row.confidence)}%{row.riskOverride?' · capped by Risk Bot':''}</small></div><div className="evidence">{evidence.map(([k,v])=><div key={k}><span>{k}</span><b>{v}</b></div>)}</div>
  <div className="botTrail"><b>Bot pipeline</b>
   {!date&&<small>No live orchestrator run yet for this security — seed a universe and run the collector to populate this.</small>}
   {date&&auditLoading&&<small>Loading bot decisions…</small>}
   {date&&!auditLoading&&audit.length===0&&<small>No audit record for this symbol on {date}.</small>}
   {audit.map((a,i)=>{const meta=registry.find(b=>b.id===a.bot);return <div className={'botStep botStep-'+a.status} key={i}><span className="botName">{meta?.name||a.bot}</span><span className="botVerdict">{a.verdict}</span></div>})}
  </div>
  <div className="chain"><b>Inference chain</b><span>Observed price + volume → Collector Bot → Matrix Bot → Classifier Bot → Risk Bot → Pattern Bot.</span><small>Indirect evidence describes a market configuration; it does not identify participants.</small></div></aside></div>}
